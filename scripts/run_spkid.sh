...
lists=lists
w=work # directorio de trabajo, podria cambiarse ej: ampliacio
name_exp=one
...
compute_lpcc() {
    ...
    for filename in $(sort $*); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2lpcc 12 18 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       # - The name of the world model will be used by gmm_verify in the 'verify' command below.
        EXEC="gmm_train -v 1 -T 0.00001 -N 5 -m 5 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train"
        echo $EXEC && $EXEC || exit 1
       ...
       compute_$FEAT $db_test $lists/final/class.test #parametrizo
